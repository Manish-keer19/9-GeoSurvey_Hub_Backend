...
export const createDistrict = async (req, res) => {
    try {
        const { name, type, serialNo } = req.body;
        if (!name || !type) {
...
export const createBlock = async (req, res) => {
    try {
        const { districtId, name, totalData, blockPercentage, surveyData, surveyPercentage, } = req.body;
        if (!districtId || !name) {
...
export const createBlocks = async (req, res) => {
    try {
        ...
        const createdBlocks = await Promise.all(blocksData.map(async (input) => {
